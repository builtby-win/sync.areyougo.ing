...
export function redactPii(text: string): string {
  ...
  const cvcPatterns = [
    /\b(cvc|cvv|security\s?code|security\s?number)[:\s]*(\d{3,4})\b/gi
  ]
  ...
  const streetSuffixes = 'St|Street|Ave|Avenue|Way|Rd|Road|Blvd|Boulevard|Dr|Drive|Ln|Lane|Ct|Court|Pl|Place|Ter|Terrace'
  // Use (?!\w) instead of \b at the end to allow for optional period
  // Included ' for cases like O'Farrell
  const addressPattern = new RegExp(`\\b(\\d+)\\s+([a-zA-Z0-9'\\.]+(?:\\s+[a-zA-Z0-9'\\.]+)*)\\s+(${streetSuffixes})\\b\\.?`, 'gi')
